...
+ function($) {
  ...
  var getDistricts = function(p, c, raw) {
    for(var i=0;i< raw.length;i++) {
  ...
  $.fn.cityPicker = function(params, raw) {
    //console.log(raw);
    ...
    return this.each(function() {
      ...
      var config = {
        ...
        onChange: function (picker, values, displayValues) {
          ...
          if(newProvince !== currentProvince) {
            var newCities = getCities(newProvince, raw);
            newCity = newCities[0].name;
            var newDistricts = getDistricts(newProvince, newCity, raw);
            ...
            if(params.showDistrict) {
              ...
              if(newCity !== currentCity) {
                var districts = getDistricts(newProvince, newCity, raw);
      ...
      if(val) {
        ...
        if(p.value[0]) {
          var cities = getCities(p.value[0], raw);
        ...
        if(p.value[1]) {
          if (params.showDistrict) {
            var dis = getDistricts(p.value[0], p.value[1], raw);
          ...
          if (params.showDistrict) {
            var dis = getDistricts(p.value[0], p.cols[1].values[0], raw);
            p.cols[2].values = dis.map(function (d) {
              return d.code;
            });
            p.cols[2].displayValues = dis.map(function (d) {
