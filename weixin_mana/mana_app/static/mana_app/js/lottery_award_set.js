...
//            alert(data);
            $.post('/mana_app/lottery/award/delete/', data, function(resp){
                if(resp.success){
                    refreshLotteryAward(resp.html);
    ...
    function addLotteryAward(data) {
        $.post('/mana_app/lottery/award/add/', data, function(resp){
            if(resp.success){
    ...
    function initUpdateLotteryAward(){
        ...
        set_optional(update_btn, 'credits');
        if ($("select[name='type'").val() == '优惠券'){
                $('div[name="credits_div"]').hide();
                $('div[name="coupon_div"]').show();
                $('.credit').attr('disabled',true);
                $('.coupon').attr('disabled',false);
            }
            else{
                $('div[name="credits_div"]').show();
                $('div[name="coupon_div"]').hide();
    ...
    function updateLotteryAward(data){
        $.post('/mana_app/lottery/award/update/', data, function(resp){
            if(resp.success){
                $('#lottery_award_modal').modal('hide');
                refreshLotteryAward(resp.html);
                swal('成功', resp.msg, 'success');
    ...
    $('select[name="type"]').on('change',function(){
        if ($("option:selected",this).val() == '优惠券'){
                $('div[name="credits_div"]').hide();
                $('div[name="coupon_div"]').show();
                $('.credit').attr('disabled',true);
                $('.coupon').attr('disabled',false);
